...
`room_events` stores schema-versioned, safe after-images rather than entity IDs that are hydrated from today's rows. PostgreSQL `NOTIFY` is only a post-commit wake-up hint: each app coalesces same-room watermarks in fixed per-room state, skips payload reads when it has no local subscribers, reads a committed contiguous range, and uses `io.local` so multiple listeners do not multiply the same Redis-adapter broadcast. The authenticated identity in server-owned `socket.data` is authoritative for that connection; Redis is a rebuildable cross-instance index. A missing Redis row is repaired from the local identity after durable membership filters every candidate room. A socket with no local authenticated identity, or a Redis identity that conflicts with it, is removed from that room and asked to register again; it does not force verified peers onto the head-only path. Before a complete payload leaves an instance, the App batches a PostgreSQL membership check. Explicitly unauthorized sockets are removed, while unavailable PostgreSQL authorization keeps sockets joined and degrades that delivery to a head-only hint.
...
This is a bounded state-transfer changelog, not Event Sourcing. Canonical tables remain authoritative and old event prefixes are pruned. There is no realtime delivery outbox and no `messageVersion`. Ordinary chat AI uses a different pattern: the placeholder, `assistant_runs`, and a narrow `task_dispatch_outbox` commit together; the App relays only `{ runId }` into BullMQ with a deterministic job ID. BullMQ owns waiting, concurrency, delayed retry, stalled recovery, and operational retention. PostgreSQL remains the only source for queued/running/finalizing/terminal business state, immutable provider output, and cost. A worker claims the exact run with a generation lease, so an old process cannot write chunks or terminal state after takeover. If the provider result is already staged, retry performs only projection and does not call the Provider again. If Redis has lost an acknowledged job or BullMQ exhausts infrastructure retries while PostgreSQL still says the run is active, a singleton reconciliation pass re-adds or retries that job. Provider invocation itself remains at-least-once across a crash after the remote service accepts a request but before RoomTalk stages the result; generation fencing still guarantees one accepted terminal projection and one internal cost settlement. Typing, presence, `ai_chunk`, voice levels, and WebRTC signalling remain transient. Worker chunks cross Redis Pub/Sub to every App, which reauthorizes local room sockets and emits through `io.local`; loss is repaired by the final durable room event. The client buffers an early transient event by `messageId`, rejects old generations, and preserves optimistic sends. PostgreSQL also rejects changing a message ID's room, preventing cross-room ghost projections.

Horizontal process ownership is lease-based. Redis heartbeat acquisition and expired-instance cleanup are atomic Lua operations: cleanup rechecks the heartbeat and removes a socket only if that instance still owns it. If a live process loses and reacquires its own Redis lease, it rehydrates session, room and browser presence for its local sockets after rechecking durable membership. A Code Agent turn has a separate PostgreSQL claim `{ roomId, turnId, ownerId, fence }`. Its start transaction acquires the next room fence, materializes a claimed queued prompt, creates the placeholder and durable turn, and marks the room running before the Socket acknowledgement. Every later execution-produced phase, transcript, steering materialization, and terminal write proves that exact claim against the live lease; an older process becomes a no-op after takeover. Completion or failure is one transaction that finalizes the current message, settles cost, updates room/session and turn state, and releases the exact lease. Recovery and retention tasks use PostgreSQL advisory locks, so one instance performs maintenance while the others keep serving. Abandoned `starting`/`steering` inputs are returned to `queued` only after their grace period and only when no live room lease exists.

Completed Codex app-server turns form an Agent-owned workspace revision DAG. A root revision anchors the room; each terminal turn adds a reversible edge from the head it started on, and each restore adds a new branch whose parent is the selected before- or after-turn revision while retaining the abandoned source head for audit. Exposing both boundaries matters: an abandoned branch's final turn remains a reachable target instead of becoming a leaf that can be inspected but never restored. Checkpoint capture still uses a private Git object database under `/tmp`, never the user's repository: only changed regular-file before/after blobs are uploaded to S3-compatible storage, with 8 MiB per-file and 64 MiB per-turn limits. Secrets, dependencies, build output, symlinks and unsupported entries are excluded.

Restoring an older checkpoint finds the lowest common ancestor of the current and target branches, undoes turn edges back to it, then reapplies target-branch edges. Every edge validates the live SHA-256 against the side it expects, so a later user edit is treated as an overlay and never silently overwritten. Conflict or unavailable data aborts the whole path: already-applied steps roll back, Codex is not forked, and the PostgreSQL head does not move. A renewed room lease blocks Agent turns, browser edits and terminal input for the complete operation. After the files match the target, `thread/fork(threadId, lastTurnId)` creates the exact selected Codex boundary; one transaction verifies the same lease, switches the room session/cursor and head, writes the boundary plus source/target/result revision IDs to the restore audit, and releases the lease. A failed fork or commit rolls the files back to the original branch; an orphaned fork is unreachable. Clearing room history or deleting the room removes its checkpoint objects after the database commit. Coco has no equivalent hidden-context fork, so exact DAG restore remains Codex app-server only.

These boundaries are covered by two-instance presence tests and real PostgreSQL 17 transaction/lease/advisory-lock tests. The release that introduced AI owner leases stopped every pre-`0006` app first because old binaries did not heartbeat the new table. Migration `0007_ai_stream_fencing` adds per-placeholder generations, and `0008_ai_stream_internal_event_filter` removes ownership-only writes from the public event sequence. Migration `0011_code_agent_turn_fencing` binds Code Agent turn records to the room lease generation. Production crossed the `0011` boundary in a maintenance cutover on 2026-07-26, with every old App and Worker stopped before the migration ran.

Runtime health has separate failure semantics. `/api/health/live` proves only that the Node process can answer. `/api/status` and `/api/health/ready` execute a real `rooms` table query, Redis `PING`, an object-storage bucket probe, and require both Socket.IO Redis adapter clients to be connected—not merely constructed. Serving dependency failure returns HTTP 503 with `status: "degraded"` and `rooms: null`; an infrastructure failure is never reported as zero rooms. Queue-only failure or an expired AI Worker heartbeat keeps the HTTP App ready because accepted requests remain durable, but status becomes `degraded` and exposes dispatch backlog plus BullMQ waiting/active/delayed/failed counts and oldest queued time.

### How the difficult paths work

- **A code-agent turn** uses the room turn itself as the durable execution aggregate rather than copying ordinary chat's BullMQ model. One PostgreSQL transaction establishes the turn and its fenced room lease before execution; fenced transactions persist each text/tool/model-step boundary; one terminal transaction converges Message, room, turn, backend session, cost, and lease. This matches an interactive, room-bound E2B daemon that accepts steer/interrupt/approval controls. The ordinary chat queue remains separate because those Provider jobs are portable competing-worker work.
