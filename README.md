...
Completed Codex app-server turns also carry a selective workspace checkpoint. RoomTalk snapshots the turn boundary in a private, isolated Git object database under `/tmp`; it does not commit to or rewrite the user's repository. At turn completion it uploads only changed regular-file before/after blobs, not a full workspace archive, to the configured S3-compatible store. Each file is capped at 8 MiB and one turn at 64 MiB of logical checkpoint data; secrets, dependency trees, build output, symlinks and unsupported entries are excluded. Restore compares the live SHA-256 with the Agent's recorded after-image. Matching files return to their before-image, while any file changed later is reported as a conflict and left untouched. A room lease blocks Agent turns, browser edits and terminal input during the operation. Codex context is restored separately by `thread/fork` through the exact pre-turn `lastTurnId`, then PostgreSQL atomically switches the room to the forked thread and records an audit row. If context or database commit fails, the saved after-images roll the files forward again. Coco does not yet expose an equivalent exact hidden-context fork, so the UI does not claim exact checkpoint restore for Coco turns.
