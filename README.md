...
Runtime health has separate failure semantics. `/api/health/live` proves only that the Node process can answer. `/api/status` and `/api/health/ready` execute a real `rooms` table query, Redis `PING`, an object-storage bucket probe, and require both Socket.IO Redis adapter clients to be connected—not merely constructed. Any unavailable dependency returns HTTP 503 with `status: "degraded"` and `rooms: null`; an infrastructure failure is never reported as zero rooms.
