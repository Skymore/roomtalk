...
Realtime and task recovery are also instance-aware. Each App process uses a unique runtime instance ID, heartbeats a Redis TTL key, and records which sockets it owns. A rolling start does not clear global presence. Singleton recovery removes only socket/session/room presence belonging to an expired instance. Code Agent uses a PostgreSQL turn claim `{ roomId, turnId, ownerId, fence }`: one transaction starts the turn and room lease, every execution-produced intermediate write validates that live claim, and one terminal transaction converges Message/cost/room/turn before releasing the exact lease. Recovery locks and rechecks candidates, so a concurrent takeover cannot be overwritten; abandoned `starting`/`steering` inputs return to `queued` only after a grace period and without a live lease. Recovery and retention loops use PostgreSQL advisory locks, so replicas can all run the same image while only one performs each maintenance pass. PostgreSQL `clock_timestamp()` is the production clock authority for assistant-run and dispatch leases when no explicit test time is provided.

Codex workspace recovery uses the same ownership rule. PostgreSQL stores an Agent-owned revision DAG and one current head per room. Restore renews the room lease, computes the current/target LCA, applies complete turn edges backward and forward, and validates the hash of the state each edge leaves. Any conflict rolls prior steps back before Codex context is forked. Only a final fenced transaction inserts the restore revision/audit and changes session, cursor and head together; old branches remain retained rather than rewritten.

Ordinary chat AI has a separate scheduling boundary. One transaction creates the streaming placeholder, `assistant_runs`, its room event, and a `task_dispatch_outbox` row. The App relay publishes only `{ schemaVersion: 1, runId }` to BullMQ with `jobId=runId`, then acknowledges that exact fenced dispatch claim. If Redis is unavailable, the row returns to pending and the accepted user request remains recoverable in PostgreSQL. After acknowledgement, a separate advisory-locked reconciliation pass compares non-terminal PostgreSQL runs with BullMQ: a missing job is recreated, and a failed or prematurely completed job is retried. This covers an empty Redis restore and exhausted infrastructure attempts without turning PostgreSQL into a second scheduler. A dedicated `ai-worker` claims the exact run, executes up to configured bounded concurrency, renews the PostgreSQL generation lease, and publishes transient events through a versioned Redis channel. Each App reauthorizes its local sockets before `io.local` emission.

BullMQ owns scheduling mechanics—waiting, concurrency, backoff, stalled-job recovery, and operational retention—but not business truth or a result backend. `assistant_runs` owns request snapshot, status, generation, immutable terminal payload, error, and usage; the final transaction also updates the Message and room cost exactly once. There is deliberately no `assistant_run_usage` ledger because the locked run transition already provides the idempotency boundary and the immutable terminal payload retains the audit evidence. This exactly-once guarantee ends at RoomTalk's PostgreSQL boundary. If a process dies after a Provider accepts a request but before RoomTalk stages the terminal payload, a replacement generation may invoke that external Provider again. Where a Provider offers a reliable idempotency key it can be added, but the portable base contract is at-least-once Provider invocation plus fenced, exactly-once terminal projection and internal cost settlement.
...
Migration `0011_code_agent_turn_fencing` stores the Code Agent room-lease owner/fence on each running turn. It is additive at the schema level but changes the write protocol: a pre-`0011` App can still write Code Agent transcript and terminal state without proving the claim. Its first deployment must therefore stop old App replicas; later compatible releases may roll after every process uses the fenced start/write/finish contract.

Migration `0013_code_agent_workspace_revision_dag` changes the Code Agent terminal/restore protocol even though its tables and columns are additive. Old writers do not append turn revisions, so its first deployment also requires a stopped-writer maintenance boundary. Backfill marks incomplete turns and ambiguous legacy restores as non-traversable barriers.
...
- real PostgreSQL tests for immutable message/room/turn/media after-images, message room immutability, wall-clock event timestamps, strict payloads, migration/checksum verification, atomic ordinary-AI placeholder/run/dispatch creation, fenced dispatch retry/ack and terminal projection, atomic Code Agent start/rollback, stale-fence rejection, workspace revision creation/restore/no-op and legacy DAG backfill, abandoned queue recovery, snapshot boundaries, concurrent writers, retention, tombstones, active/expired leases, and singleton advisory locks. GitHub CI provisions PostgreSQL 17 and Redis 7 under Node 24.18, supplies both `ROOM_EVENT_TEST_DATABASE_URL` and `BULLMQ_TEST_REDIS_URL`, and therefore exercises trigger transactions and real queue retry/deduplication;
