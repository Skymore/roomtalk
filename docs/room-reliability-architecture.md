...
The failure is not left for a future restart. An in-process terminal-persist reconciler retains the exact terminal Message and retries with bounded exponential backoff until PostgreSQL accepts it and creates the normal `messages.upserted` event. Every stream owner also heartbeats a PostgreSQL lease. Periodic singleton recovery changes a `streaming` placeholder to a normalized error only when its owner is absent/expired and there is no queued/running assistant run backed by a pending/processing outbox event. Recovery sets `is_error/updated_at`, clears ownership, and removes expired owner leases no longer referenced by a streaming message. A replacement process therefore cannot kill either a live stream or a recoverable durable job before the worker resumes.
...
Realtime and task recovery are also instance-aware. Each process uses a unique runtime instance ID, heartbeats a Redis TTL key, and records which sockets it owns. A rolling start does not clear global presence. Singleton recovery removes only socket/session/room presence belonging to an expired instance. Code Agent turns and sandbox recovery queries exclude rows protected by an unexpired fenced room lease. Recovery and retention loops use PostgreSQL advisory locks, so replicas can all run the same image while only one performs each maintenance pass. PostgreSQL `clock_timestamp()` is the production clock authority for AI-owner and outbox leases when no explicit test time is provided.

AI worker mode atomically creates the streaming placeholder, `assistant_run`, and `ai.run_requested` outbox row in one PostgreSQL transaction. The outbox defaults to claim-one/execute-one because claim time starts the lease while the executor is serial; claiming ten rows would allow the waiting nine to expire before their renewal begins. A future throughput increase must use bounded concurrency and renew every claim from claim time.

This remains a safer intermediate model rather than the final AI aggregate. Terminal truth is still distributed across message, run, outbox, cost projection, owner lease, and the process-local reconciler. The next convergence step promotes `assistant_runs` to the sole durable execution job with generation, database lease, request/terminal payload, and an idempotent usage ledger. That work does not require redesigning the stable room-event changefeed.
...
- real PostgreSQL tests for immutable message/room/turn/media after-images, message room immutability, wall-clock event timestamps, empty public membership signals, member-event privacy repair, strict payload rejection, secret exclusion, migration/checksum verification, atomic placeholder+run+outbox creation, startup protection for recoverable durable jobs, snapshot boundaries, idempotency, rollback, concurrent writers, retention, direct tombstone convergence across 600 earlier events, first-event byte limits, active/expired Code Agent and AI-owner leases, and singleton advisory locks. GitHub CI provisions PostgreSQL 17 under Node 24.18 and always supplies `ROOM_EVENT_TEST_DATABASE_URL`, so this suite cannot silently skip there;
