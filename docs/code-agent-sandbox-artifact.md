...
Verified against `master` and the production non-secret runtime pins: 2026-08-10
...
- Codex app-server retry notifications that remain non-terminal only when the pinned protocol explicitly sets `willRetry: true`, without exposing raw provider error details
...
```bash
docker build -t roomtalk-code-agent:roomtalk-code-agent-2026-08-10-codex-retry-notification-v1 /tmp/roomtalk-code-agent-sandbox-context
```
...
node scripts/code-agent/build-e2b-template.mjs \
  --clean \
  --template roomtalk-code-agent-2026-08-10-codex-retry-notification-v1 \
  --publish
```
...
CODE_AGENT_E2B_TEMPLATE_ID=roomtalk-code-agent-2026-08-10-codex-retry-notification-v1
E2B_API_KEY=...
CODE_AGENT_ARTIFACT_MODE=production
CODE_AGENT_ARTIFACT_VERSION=roomtalk-code-agent-2026-08-10-codex-retry-notification-v1
