...
The same rule protects work. Code Agent is an interactive room-bound execution path rather than a BullMQ job. One PostgreSQL transaction creates its prompt/placeholder/turn and acquires a `{ roomId, turnId, ownerId, fence }` lease before acknowledgement. Every later execution-produced turn write proves that claim, and one terminal transaction converges message, cost, room/session, turn, and exact lease release. A stale process therefore cannot keep writing after takeover. Recovery locks and rechecks candidates; it recovers a turn/sandbox or restores an abandoned `starting`/`steering` input only when no matching live lease exists. Ordinary chat AI has a different boundary: a queued run has durable dispatch intent, an active Worker owns a generation lease, and a `finalizing` run already has an immutable terminal payload. App restart therefore cannot kill that job. BullMQ retry or a replacement Worker claims a higher generation only after the old lease is gone, while stale transient and terminal writes fail their fence.

Completed Codex app-server turns also advance an Agent-owned workspace revision DAG. A restore can target either side of a completed turn, so an abandoned branch's final after-turn state is still reachable. It holds and renews the same room lease, finds the LCA of current and target branches, applies whole turn checkpoints backward and forward with hash validation, then forks Codex at the selected context boundary. Conflict anywhere reverses already-applied steps and leaves both context and the room head unchanged. The final transaction records the target boundary plus source/target/result revision IDs and moves the room head; the abandoned branch remains addressable. Checkpoint files stay selective before/after blobs in S3-compatible storage rather than full workspace snapshots or commits in the user's repository.
...
Migration `0003_room_events_immutable_after_images` takes table locks so no business write can land between replacing the writer and deleting legacy ID-only events. Schema mutation is no longer an App-startup responsibility: a dedicated Compose migration service (or a Kubernetes pre-deploy Job) acquires the transaction advisory lock and applies only missing immutable migrations. `schema_migrations` stores a SHA-256 checksum; modifying already recorded SQL is a hard failure. `POSTGRES_SCHEMA_SQL` is frozen as the `0000` bootstrap for a new database, while every later change gets a new migration ID. App startup only verifies the IDs and checksums and refuses readiness if the migration job was skipped.

The room-event migration preserves every stream `head_seq`, clears nondeterministic history, and sets active `min_available_seq = head_seq + 1`; an old cursor therefore snapshots once and resumes without a sequence reset.
...
Migration `0013_code_agent_workspace_revision_dag` adds room head, `root`/`turn`/`restore` nodes, turn parent/result links and restore source/target/result audit IDs. It backfills old history conservatively: incomplete turns and legacy partial or non-current restores become non-traversable barriers instead of fabricated reversible state. A pre-`0013` App can finish a turn without adding its revision edge, so the first deployment must stop old App writers before migration and restart only the DAG-aware image.
...
The AI placeholder, `assistant_runs`, room event, and dispatch row commit in one PostgreSQL transaction. The relay then enqueues a minimal `{ schemaVersion: 1, runId }` job with `jobId=runId`; duplicate relay delivery is harmless. A queue outage returns the dispatch row to pending, so an accepted request cannot disappear between PostgreSQL and Redis. Once a dispatch has been acknowledged, an advisory-locked repair loop periodically checks only active PostgreSQL runs. It recreates a missing deterministic job and retries failed or prematurely completed jobs, while leaving waiting/active/delayed jobs alone. The dedicated Worker fetches the request from PostgreSQL, claims that exact run with a new generation, and continuously renews its owner lease while the Provider executes.

`assistant_runs` is the only business aggregate: it owns the request snapshot, generation, lease, immutable terminal payload, status, error, and usage. BullMQ state is not queried to answer whether a run succeeded, and its result backend is unused. If a retry sees a staged terminal payload, it projects Message/run/cost only and does not call the Provider again. The locked terminal transition itself prevents duplicate internal cost, so a second `assistant_run_usage` ledger is intentionally absent. A crash before terminal staging can still repeat an external Provider request after takeover; therefore Provider invocation is documented as at-least-once, while accepted terminal projection and RoomTalk cost settlement remain fenced and exactly once.
...
Kubernetes is optional. On one MacBook, Compose is the smaller operational surface; Kubernetes does not make one physical host highly available. Portability comes from the shared App/Worker image, PostgreSQL schema/dump/WAL contracts, BullMQ's Redis contract, separate `REDIS_URL` / `QUEUE_REDIS_URL` boundaries, and the S3 API. Realtime/cache keys may warm naturally after migration. A zero-downtime queue move should drain or migrate active jobs; a maintenance restore may start with an empty queue after old Workers stop, because pending dispatch is relayed and acknowledged-but-active runs are reconstructed from PostgreSQL. Current production uses SeaweedFS, the rollback deployment uses Tigris, and AWS will use S3 without changing object keys or APIs.
...
Run `node scripts/backup-local-production.mjs` for a consistent maintenance backup. It briefly stops the edge, app, and object store, then writes a matching PostgreSQL custom archive and SeaweedFS data snapshot before starting those exact stopped containers again. The recovery path deliberately uses `compose start`, not `compose up`, so a backup cannot reconcile a newer Compose definition against an older image and become an accidental deployment. Queue Redis is operational state rather than a business backup source: restoring PostgreSQL into an empty queue is supported because pending dispatch is relayed and acknowledged-but-active dispatch is rebuilt by reconciliation. Local backups are not off-host backups; production still needs encrypted external copies and restore drills.
...
Health is not one optimistic boolean. `/api/health/live` is an App process probe. `/api/status` and `/api/health/ready` test a real table read, realtime Redis, S3, and the Socket adapter. A serving failure returns HTTP 503 and `rooms: null`; queue-only failure keeps the App ready but reports `degraded` and deferred dispatch. Every `ai-worker` renews a shared queue-Redis TTL heartbeat, so a dead Worker also degrades public status even while Redis itself still answers. Status exposes PostgreSQL pending/processing dispatch plus BullMQ waiting/active/delayed/failed counts and oldest queued time. `ai-worker` retains its own health probe for PostgreSQL, queue Redis, transient Redis, and local worker state. AWS should map these separately and alert on host/disk, expired Worker heartbeat, and sustained queue age.

GitHub CI uses Node 24.18, starts PostgreSQL 17 and Redis 7, and sets `ROOM_EVENT_TEST_DATABASE_URL` plus `BULLMQ_TEST_REDIS_URL`. Trigger/transaction/fencing tests and real BullMQ dedupe, processor retry, missing-job rebuild, and exhausted-job reconciliation tests therefore fail the job instead of silently skipping.
