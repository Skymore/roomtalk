...
An authenticated account can remove its Google login identity with `DELETE /api/auth/google`, but only after a User ID password has been set. Disconnecting deletes the Google identity and its profile fields; it does not delete the account, current app session, rooms, membership, credits, usage history, or persisted roles. The transaction also writes an immutable `account_identity_events` audit record. This means a platform administrator keeps the database-backed `admin` role after disconnecting Google, while future Google sign-in is unavailable until the identity is linked again.
...
Accounts created before account-level usage settlement can reconcile their historical lifetime usage from durable assistant-run and Code Agent observability records. The backfill is read-only by default; inspect it with `npm run backfill:lifetime-ai-usage`, then apply it once with `npm run backfill:lifetime-ai-usage -- --execute`. It inserts deterministic, immutable usage rows and recomputes `lifetime_usage_usd` from the ledger total without retroactively consuming current credits. Re-running it is safe and reports an already-current result.
...
- The primary `ruit.me` Tunnel uses ignored `CLOUDFLARE_TUNNEL_CONFIG_FILE` and `CLOUDFLARE_TUNNEL_CREDENTIALS_FILE` paths. The separately managed `ai-chat.wenlin.dev` Tunnel reads `CLOUDFLARE_WENLIN_TUNNEL_TOKEN` from the Keychain environment; never commit or print that token.
- `local-production.mjs` automatically verifies all seven production services after detached startup and reports host/Docker disk use. `ROOMTALK_MIN_HOST_FREE_GB`, `ROOMTALK_DOCKER_RAW_WARN_GB`, `ROOMTALK_DOCKER_RAW_PATH`, `ROOMTALK_PUBLIC_STATUS_URL`, and `ROOMTALK_WENLIN_PUBLIC_STATUS_URL` tune that local operator check.
