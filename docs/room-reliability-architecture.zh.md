...
当前 Socket.IO 连接完成注册，并且服务端已经确认当前 session epoch 所选择房间的 membership 后，房间才进入 `ready`。在此之前，恢复流程可以显示保存的房间、缓存消息，以及同一房间、同一 client 最近一次确认的权限快照。首次访问、client 已变化或本地没有匹配快照时，成员操作继续锁定。页面在 controller 恢复期间仍可使用，任何依赖 socket 的操作都会先等待当前房间重新 ready，再发送到服务端。
...
跨模块传递的命名值有四个。数据版本有两个：`roomVersion` 排序房间对象，`messageVersion` 排序持久消息历史。`sessionEpoch` 和 `messageSyncRequestId` 是当前标签页内部的控制标记。前者判断异步结果是否还属于当前 socket 会话，后者只负责唤醒消息同步 effect。

消息 hook 内部还有一个私有的 `mutationRevision`。每次实时消息变化都会推进它，让 history response 判断请求在途期间页面上的窗口有没有动过。它不会发给服务端，也不会持久化，更不代表服务端版本。

排查数据新旧时只比较同名值。`roomVersion` 不会和 `messageVersion` 比较，控制标记也不会写入持久化数据。这样可以先回答一个具体问题，再找对应的数字，不需要把几个值都理解成相同含义的 version。
...
| 当前房间与按 client 隔离的 retained permission snapshot | [`appPersistence.ts`](../client-heroui/src/utils/appPersistence.ts) |
...
| 消息授权、mutation 与 history invalidation | [`messageHandlers.ts`](../server/src/socket/messageHandlers.ts)、[`aiHandlers.ts`](../server/src/socket/aiHandlers.ts) 和 [`roomAuthorization.ts`](../server/src/socket/roomAuthorization.ts) |
...
假设用户上次停在房间 `nZDcDhQEcu`，关闭应用后又重新打开。`MessagePage` 会从 localStorage 读出保存的 `Room`、view 和最近一次确认的 `RoomPermissions`。只有 permission snapshot 的 `roomId` 与 `clientId` 都和本次恢复目标一致时，页面才接受它。房间名称、header、消息区域和此前获得授权的控件可以立即显示，`useRoomMessageEvents` 同时开始读取内存窗口或 IndexedDB 缓存。

当前 socket 的 membership 仍未验证，本地房间与权限也可能过期。依赖 socket 的操作会先等待 `ensureRoom`，服务端随后在操作边界检查当前权限。高风险设置和 workspace 写入继续等到 session ready。没有匹配 permission snapshot 时，全部成员操作都会等第一次 join 成功后再开放。
...
Register 的作用是把这些持久浏览器身份绑定到当前 socket。任何要求已注册状态的操作都会等待 controller 中共享的 registration promise。注册过程不读取 room list。页面在注册完成后分别请求 owned room 和 saved room snapshot，因此列表存储不会占住 register ack，也不会把它拖成 `Timed out while registering client`。
...
新确认的权限会替换 localStorage 中的 retained copy。离开或删除房间、切换当前房间、清理该 client 的持久缓存，以及服务端明确拒绝 access 时，这份 copy 都会被删除。

### 6. 消息层单独完成 history 对账

Session ready 本身不会替换消息窗口。消息 hook 会先等缓存 hydration 结束，记录当前 canonical `messageVersion` 与私有 `mutationRevision`，再用唯一 `requestId` 发送 `get_room_messages`。服务端通过这次请求自己的 ack 返回消息页。客户端只有在它仍是最新 replace request，并且两个 boundary 都没有变化时才会接受。
...
sequenceDiagram
    ...
    Page->>Page: 恢复房间、缓存消息和匹配的 retained permissions
    ...
    Server-->>Messages: Ack(requestId, 带版本的 history page)
...
| `connecting` | 已经有 desired room，但还没有可用 socket ID | 保留 room shell；只有保存快照同时匹配 room 与 client 时才沿用 retained access |
| `registering` | Transport 已连接，当前 socket 的 identity binding 尚未完成 | 等待 register ack，同时维持上一行所述的 UI 边界 |
| `joining` | Socket 已注册，目标 membership 尚未提交 | 新目标或无法识别的目标保持锁定；同房间恢复可以沿用匹配的本地或内存 access |
...
`mutationRevision` 没放进表里，因为它不是共享状态。房间消息 hook 挂载时它从零开始，只用于判断某一次请求期间，这个标签页的实时窗口是否发生过变化。
...
服务端为每个 socket 串行执行注册、join、leave、重新注册和 disconnect cleanup。这样可以保证同一 socket 上较早开始的异步操作不会在较晚操作之后提交。会改变 access 的 membership 操作还会经过 room 级队列和 Redis lease。进程内队列负责本 worker 的顺序，Redis lease 把同一临界区扩展到多个 Socket.IO worker。Join、删除、移除成员、管理员变更和 ownership transfer 不会再基于彼此过期的快照同时提交。

如果没有 socket queue，房间 A 的慢 join 可能在之后发出的房间 B join 后面才完成，最后反而让 A 成为服务端记录的 membership。Queue 让最终状态遵守请求顺序。Room lock 解决另一种竞态：管理员通过 worker A 移除成员时，该成员可能正通过 worker B join。Join 添加临时 presence 后会重新检查 membership，因此串行执行的 removal 会在成功 ack 发出前生效。Redis lock 使用短租约、定时续租和 token 校验释放；持有者崩溃后锁会自动过期，旧持有者也不能误删后来创建的新锁。
...
Join ack 包含完整 `Room`、当前 `RoomPermissions` 和成员数。重复加入当前房间是幂等操作。注册过程没有主动 room list 读取；owned room 与 saved room snapshot 各自使用独立 ack 请求。
...
Join 成功后，页面用通过版本检查的 canonical room 替换 shell，并安装当前权限。之后的权限刷新、timeout、transport 恢复失败或浏览器完整重启时，`MessagePage` 都会为当前房间保留这份快照。持久副本同时绑定准确的 room 与 client，切换账号不会继承另一个 client 的控件。服务端明确拒绝 identity 或 room access 时才会清除它，包括 client auth 无效、要求重新登录、房间不存在或 access 被移除、密码无效，以及 code-agent access 不可用。确认房间不存在或 access 已被移除时，还会进入 room removal 路径并清除 shell。
...
消息缓存不承担持久化职责。缓存结构变化时，客户端使用新的数据库名称并删除旧库，然后从服务端重新读取消息。当前消息窗口保存在 `roomtalk-message-cache-v3`。同一房间在一个 event loop tick 内的连续写入会合并，只把最新窗口写入 IndexedDB；trim 也有节流，不会每次写入都扫描。待写任务绑定创建它的 client，真正落库前如果 active client 已切换，这次写入会直接丢弃，不会进入另一个账号的缓存库。

History request 由独立 effect 发出。只有房间 session ready，并且 `messageSyncRequestId` 或 reconciliation retry nonce 变化时才会执行。每个请求都有唯一 `requestId`，携带 canonical 本地 `messageVersion` 作为 `baseMessageVersion`，并读取最近 80 条消息。Response 通过该请求自己的 Socket.IO ack 返回，不再走共享 `message_history` event，因此两个重叠请求不会拿走对方的 payload。

可以用之前出现过的“房间恢复了，但新消息不显示”来理解这里的竞态。客户端带着 canonical version 3462 发出 history request，同时记录 `mutationRevision: 0`。Response 尚未回来时，页面先收到一条 `new_message`，可见窗口变化，私有 revision 变成 1。延迟 response 可能仍然来自这条消息之前的 snapshot。如果客户端直接用它 replace 当前列表，刚收到的实时消息就会被擦掉。

实时事件会更新可见窗口并推进 `mutationRevision`，但不会伪造新的 canonical `messageVersion`。History response 到达后，客户端检查 `requestId`、服务端回传的 `requestedMessageVersion`、请求开始时记录的 mutation revision，以及服务端 `messageVersion`。出现更新的 replace request、canonical boundary 已变化、本地窗口已变化，或服务端版本更旧时，这份 response 都会被视为 stale。客户端保留当前内容并重新对账，最多重试三次。

在这个例子里，`[room-messages] history-response` 会记录 `requestedMessageVersion: 3462`、`requestedMutationRevision: 0`、`currentMutationRevision: 1` 和 `decision: "ignored"`。下一次请求记录新的本地 revision，同时仍把 3462 视为最近一次接受的服务端 boundary。等 response 同时通过两个检查后，它的服务端 `messageVersion` 才会成为新的 canonical boundary。

有些 mutation 会替换或截断整段后缀，无法用一条普通消息 delta 表达。AI retry 和 edit-and-ask 就属于这种情况。持久化成功后，服务端 broadcast `message_history_invalidated`。房间内每个客户端再通过 ack 协议请求自己的 canonical page。失效事件只说明“需要对账”，不会携带另一份可能与请求竞态的完整 history payload。
...
媒体读取同时遵守 retained-access 展示边界和独立的服务端授权边界。恢复时有匹配的 saved access，页面可以在 socket membership ready 前读取缓存媒体或申请 signed URL；没有匹配 access 时则不会执行。HTTP endpoint 每次都会检查 client auth token、当前持久 room access、room ID 与 asset 归属，并不会信任客户端保留的权限快照。
...
如果消息还没有任何可用的本地 URL 或 signed URL，无法识别 access 的 shell 会等到第一次 join 成功。具有匹配 retained access 的恢复流程可以在 socket 恢复期间申请新的 15 分钟 read URL。这里返回 403 表示请求当下的持久 room access 校验失败，客户端会展示失败，而不会把旧 permission snapshot 当成授权。
...
`MessagePage` 会先同步推进 `currentRoomRef`，再把带版本守卫的 React state update 入队。Ack 和 broadcast 即使在同一次 React commit 前相继到达，也能看到同一个最新房间。增量房间更新对 active room、owned room list 和 saved room list 使用同一守卫。

Owned room 与 saved room list 都采用“一次 snapshot，之后持续接收 delta”。`get_rooms` 和 `get_saved_rooms` 通过 ack 返回初始 snapshot；`new_room`、`room_updated`、`room_removed`、`saved_room_added` 和 `saved_room_removed` 负责后续收敛。如果 snapshot 在途时收到 delta，页面会先记录它，再把它重放到返回的 snapshot 上，迟到 snapshot 因而不能复活已删除房间，也不能覆盖新房间。Socket replacement 后会重新请求 snapshot，补齐离线期间错过的事件。
...
服务端只会在持久化成功后 broadcast。失败的写入不会发布 durable store 中不存在的状态。Ack 与 broadcast 重复送达时，相同的 `roomVersion` 和重复 removal delta 会幂等收敛。Save、unsave 和 delete ack 也会立即更新发起页面；private channel delta 负责收敛同一 client 的其他标签页。

消息发送遵守同一分工。Send ack 把服务端实际保存的 canonical message 返回给发起者，`new_message` 则向房间 broadcast，让其他参与者收到。History read 是请求者私有的 request/ack。整窗失效的 mutation 只 broadcast `message_history_invalidated`，由每个参与者自行请求带版本的 snapshot。
...
服务端在操作发生时重新做权限判断，覆盖消息发送、媒体上传初始化与完成、消息编辑和删除、房间管理以及 code-agent access。客户端早先收到的 permission snapshot 可以在恢复期间保持界面稳定，包括浏览器完整关闭后重新打开的情况，但不能自行授权后续操作。依赖 socket 的操作会先恢复当前 membership，再依赖服务端的即时检查。
...
断线、transport 变化和 ack 超时会在重试预算内继续恢复。房间外壳与缓存内容保持可见。没有匹配 saved access 的恢复流程继续锁定；已经确认过的房间在 transport 中断或浏览器完整重启后仍保留 UI access，并允许下一次操作触发新的恢复。预算耗尽后进入 `unavailable`，页面提供显式重试，但不会只因为 timeout 就把它当成 access 被撤销。
...
Register 与 join failure 带有稳定的机器可读 code，例如 `CLIENT_LOGIN_REQUIRED`、`ROOM_NOT_FOUND`、`ROOM_ACCESS_REMOVED` 和 `ROOM_PASSWORD_REQUIRED_OR_INCORRECT`。Controller 根据 code 判断重试是否有意义，`MessagePage` 也根据 code 进入移除或密码流程。错误文本只负责展示，修改文案或翻译不会改变恢复行为。
...
一次排查应同时关联 `roomId`、`socketId`、`sessionEpoch`、`messageSyncRequestId`、`requestId`、`requestedMessageVersion`、`mutationRevision` 和 `messageVersion`。正常的本地房间恢复通常按以下顺序出现：
...
- [`MessagePage.test.tsx`](../client-heroui/src/pages/MessagePage.test.tsx) 覆盖本地恢复、URL 与手动切房竞态、lifecycle resume、重连锁定、回退、完整对象替换、list snapshot/delta 重放、room version、ack 收敛和 posting refresh。
- [`useRoomMessageEvents.test.tsx`](../client-heroui/src/hooks/useRoomMessageEvents.test.tsx)、[`MessageList.test.tsx`](../client-heroui/src/components/MessageList.test.tsx) 与 [`messageHistoryCache.test.ts`](../client-heroui/src/utils/messageHistoryCache.test.ts) 覆盖缓存 hydration、实时消息与 history 的竞态、invalidation 驱动对账、分页、批量持久化、内容连续性和操作锁定。
...
- [`roomHandlers.test.ts`](../server/src/socket/roomHandlers.test.ts) 覆盖注册与 list 请求分离、membership mutation 串行化、幂等 rejoin、access removal、房间删除、delta broadcast 和 disconnect cleanup。
- [`messageHandlers.test.ts`](../server/src/socket/messageHandlers.test.ts) 覆盖 history 授权，以及消息 mutation 的 ack 和 broadcast。
- [`storeContract.test.ts`](../server/src/repositories/storeContract.test.ts) 与 [`redisStore.test.ts`](../server/src/repositories/redisStore.test.ts) 覆盖单调 room/message version、持久 membership、跨实例 room access lock、presence、缓存有效性和媒体历史。
