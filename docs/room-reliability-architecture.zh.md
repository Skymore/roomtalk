...
任何完整 durable payload 发出前，实例先从 Redis 或认证后由服务端保存的 `socket.data` 解析本机 socket 的 client ID，再对去重后的 client ID 做一次 PostgreSQL membership 查询。结果分三种：authorized 可以收完整 payload；明确 unauthorized 的 socket 先收到 `room_removed` 并离开；身份无法解析、身份冲突或 PostgreSQL 授权不可用时，不移除任何连接，只把这次投递降级为 head-only，并记录 authorization-unavailable metric。这样 `members.changed` 仍是权限屏障，但基础设施故障不会被误判为成员被删除。
...
Realtime 与任务恢复也按实例划分。每个进程生成唯一 runtime instance ID，在 Redis 续租 TTL heartbeat，并记录自己拥有的 sockets。滚动启动不会再清空全局 presence；singleton recovery 只删除已过期实例名下的 socket/session/room presence。Code Agent turn 与 sandbox recovery 查询会排除仍受未过期 fenced room lease 保护的记录。Recovery 与 retention loop 都用 PostgreSQL advisory lock，因此所有副本可以运行同一镜像，但每轮只有一个实例执行维护。若某进程暂停到 lease 过期，它就被视为失去所有权；权威依据是 durable lease，不是进程内存或 hostname。
