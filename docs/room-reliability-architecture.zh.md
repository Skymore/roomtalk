...
这些数字本来就不会彼此对齐。一个完全健康的页面可能同时显示 `sessionEpoch: 2`、`resyncRevision: 7`、`roomVersion: 418` 和 `historyVersion: 3462`。Socket 也许替换过两次，标签页做过多次前台对账，而房间已经积累了几千次消息 mutation。让这些值相等没有任何业务含义。
...
事件沿着固定方向传递。以 `visibilitychange` 为例，它先到 `MessagePage`，页面调用 `roomSessionController.resume("visibility")`。Controller 返回正在执行的 room completion，或者发布新的 resync revision。React 观察 snapshot，消息 hook 再判断是否需要请求 history。Lifecycle handler 自己不会发送 `register`、`join_room` 或 `get_room_messages`。
...
成功的 join ack 包含完整 `Room`、当前 `RoomPermissions` 和成员数。Controller 会确认这个 ack 仍属于当前 desired room、epoch 和 socket，然后发布 `phase: "ready"`，保存结果，并推进 `resyncRevision`。
...
Session ready 本身不会替换消息窗口。消息 hook 会先等缓存 hydration 结束，读取当前本地 `historyVersion`，再发送 `get_room_messages`。服务端从持久化存储返回带版本的消息页。请求在途期间如果有新的实时 mutation 改变了本地窗口，客户端会拒绝这份旧 response 并再次对账。

这也是 join ack 推进 `resyncRevision` 的原因。Membership 先进入 ready，消息历史随后使用自己的 version 完成比较，二者没有共用同一个 revision。

### 7. 后续断线只重做必要步骤

Socket A 断线后，controller 进入 `retrying`。页面会锁住新的成员操作，同时保留房间外壳、消息、滚动位置和已经加载的媒体。Socket B 连接后，controller 推进 `sessionEpoch`，注册 socket B，重新加入相同 desired room，并在新 join 成功后推进 `resyncRevision`。

下面的时序图是这两条链路的索引。重试预算、supersession 和仅发生 history resync 的前台恢复，会在后续章节继续说明。
...
这两种 resume 在 UI 上可能很相似，但日志完全不同。回到仍使用同一个 ready socket 的标签页，只会推进 `resyncRevision`。如果手机系统已经挂起 socket，恢复时会出现新 socket ID、新 epoch、register 和 join。400 ms reconnect indicator 只会在第二条路径持续到足够明显时出现。
...
在这个例子里，`[room-messages] history-response` 会记录 `requestedHistoryVersion: 3462`、已经更新的 current version，以及 `decision: "ignored"`。下一次请求使用新的本地 version 作为 base。等 response 描述的 boundary 与页面当前窗口一致后，客户端才能安全地确认或替换它。
...
消息变化也会更新 `lastActivityAt` 等房间元数据，因此它们需要同时推进 room 和 message 两个版本。房间列表用完整 room object 排序活动时间，消息层则继续使用 `historyVersion` 对账自己的窗口。
...
例如，已经 ready 的 socket 回到前台时，只应改变 `resyncRevision` 并产生新的 `history-request`。`sessionEpoch` 和 socket ID 保持不变，日志中也不应出现 `registration-attempt` 或 `join-attempt`。如果这条 trace 中重新发生 join，说明 controller 已经无法沿用旧 membership，常见原因是 socket ID 变化，或者前一次 session 从未真正进入 ready。
