...
错误使用同一思想的确定性版本。`ai_stream_error` 必须携带 `messageId`、`error` 与 `persisted`。正常路径先持久化完整、用户可见且 `status: error` 的 Message，再发送 `{ persisted: true, message }`，其中 Message 就是同一条安全 after-image。如果最终持久化本身失败，服务端发送 `{ persisted: false }`；浏览器会立刻把已有 placeholder 改成终态 error，把这层本地 error overlay 保留到 replace snapshot 之后，并安排 durable recovery，而不是让 UI 永远停在 `streaming`。错误若早于 placeholder 到达，则按 `messageId` 暂存。后续 durable terminal after-image 会清除 overlay 并重新成为事实源。`aiStreamOwnerId` 等内部恢复字段在 fast path 发出前会被删除。
...
后台任务默认每小时执行，保留 7 天且每房间最多 10,000 个事件。`room_events.created_at` 使用 `clock_timestamp()`，所以 retention 依据事件真正 materialize 的时间，而不是长事务开始时间。任务只删除连续旧前缀并推进 `minAvailableSeq`；删除房间的事件全部过期后再移除 stream。事件无需“合并”回 messages，因为当前状态已在原写事务中同步更新。

## 多实例通知

每个 app 实例都 `LISTEN room_event_committed`，从 PostgreSQL 读取已提交事件，再用 `io.local.to(roomId)` 发送 `room_event_available {roomId, headSeq, events?}`。同房间通知会合并成 pending seq range，因此突发通知不会为每条 event 各排一个数据库读取和 broadcast promise。只有完整连续 range 的序列化结果不超过 `ROOM_EVENT_FAST_PATH_MAX_BYTES`（默认 256 KiB）时才带 payload；读取不完整、失败或超限时统一退化为一个最高水位 head-only hint。

任何完整 durable payload 发出前，实例都会重新用 PostgreSQL 校验本机 room sockets 的成员身份，并先移除已经无权访问的 socket。这就是 `members.changed` 的 authorization barrier：成员删除事务提交后，即使 Redis presence cleanup 还没结束，被删除连接也收不到后续 Message after-image。Head-only hint 不需要携带数据，后续 GET 本身还会重新鉴权。
...
- broadcaster/reducer/state-machine tests 覆盖精确已提交 payload、local-only fan-out、成员 authorization barrier、突发水位合并、listener generation 替换、字节超限 fallback、fast path 零补拉、recovery 与 prepend 竞态、窗口删空恢复、大 gap snapshot、cache 恢复、过期、数据库回退时重置水位与 gap target、reset snapshot 期间保留新通知、坏 payload snapshot、删除、turn、room metadata、提前到达 AI 临时事件、AI 持久/未持久终态，以及临时 AI 更新期间保留 optimistic send；
- 不依赖数据库的严格 V1 payload 单测覆盖全部 event type、空 AI/media content、缺失/额外字段、room 绑定、重复 ID 与退役 ID-only payload；
- 真实 PostgreSQL 测试覆盖不可变 message/room/turn/media after-image、message 房间不可变、wall-clock event timestamp、空公共成员 signal、旧成员事件隐私修复、严格 payload 拒绝、secret 排除、migration 切换、快照、幂等、回滚、并发、room metadata 单调性、retention 与删除授权。GitHub CI 会启动 PostgreSQL 17 并强制提供 `ROOM_EVENT_TEST_DATABASE_URL`，因此该套件不能在 CI 静默 skip；
