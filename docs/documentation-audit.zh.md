...
- PostgreSQL `NOTIFY` 是提交后的唤醒 hint。每个监听中的 App 用固定状态合并同房间水位；本机无订阅时跳过 payload 读取，完整 payload 前批量重新鉴权，再用 `io.local` 通知自己的客户端。授权依赖不可用时只降级 head-only，不会误删用户。
- 连续的 Socket payload 是低延迟 fast path。缺失 payload 从 PostgreSQL replay；单条事件超限、差距超过 500 events 或 cursor 过期时读取 repeatable-read snapshot。Deleted stream 直接返回最终 tombstone，因为它已没有 snapshot。
- Per-room 的 `idle/replay/replace/prepend` 状态机让恢复优先于分页，实时 mutation 会使在途 prepend 失效。处理 `CURSOR_AHEAD` 时同时清除旧 head 与 gap target；过期 `beforeMessageId` 改做 replacement snapshot。
...
- `ai_stream_error` 会声明 `persisted`。正常路径携带同一条已持久化 safe Message；即时终态保存失败时使用 `persisted: false`，客户端立刻终止 placeholder，服务端进入进程内指数退避重试。PostgreSQL owner lease 只有在 live owner 过期后才允许接管。
- Redis presence 由唯一 runtime instance 加 TTL heartbeat 持有；Code Agent recovery 尊重未过期 fenced lease；recovery/retention 用 PostgreSQL advisory lock。滚动启动不再清理其他活实例的用户或任务。
- 健康状态明确区分进程 liveness、依赖 readiness 与 degraded。Readiness 会真实检查 PostgreSQL、Redis、对象存储和 Socket adapter；数据库失败时返回 `rooms: null`，绝不会伪装成 `rooms: 0`。
...
- GitHub CI 使用 Node 24.18、启动 PostgreSQL 17，并强制运行真实 trigger/transaction/lease/advisory-lock suite，不允许静默 skip。
