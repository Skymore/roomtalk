...
只有房间创建者能发送 Coco AI 消息，硬编码不可配置。

### 方案：可配置的 Coco 访问权限

Room 上新增字段 `cocoAccess: 'owner' | 'admin' | 'member'`，默认 `'owner'`：
...
客户端设置的 `maxContextMessages` 对 Coco 无作用。对话长了之后 priorMessages 无限增长，导致 token 浪费和可能的上下文溢出。
...
## 问题五：Coco edit/plan mode 的作用域与持久化

### 现状

`edit/plan` mode 现在是客户端本地状态：

- `CodeAgentRoomView` 用 `localStorage["roomtalk_code_agent_mode_${roomId}"]` 记住当前房间的本地选择
...
- 前端不再用 `localStorage["roomtalk_code_agent_mode_${roomId}"]` 作为权威状态
- `room_updated` 广播后所有成员 UI 同步显示当前房间 mode

同时，最终执行时解析出的 `resolvedMode` 仍必须作为**本次 Coco turn 的事实**持久化。
...
2. **长期**：如果引入 `coco_turns` / `assistant_runs` 统一表，则把 `codeAgentMode` 放到 turn metadata，message 通过 `turnId` 关联

### 服务端行为

`CocoSessionService.startTurn()` 应该先解析并校验 mode：
...
- 每个 Coco AI turn 或工具组显示一个小 badge：`Plan` / `Edit`
...
2. Redis / Postgres store 持久化该字段，创建 Coco room 时默认写 `COCO_DEFAULT_MODE` 或 `plan`
3. 新增 socket/API 更新房间 code-agent mode，复用房间权限校验
4. `CodeAgentRoomView` 从 `currentRoom.codeAgentMode` 读取 mode，删除本地 `roomtalk_code_agent_mode_${roomId}` 状态
5. `MessageInput` / `MessageList` 不再把 `codeAgentMode` 当作每次请求的权威参数
6. `CocoSessionService.startTurn` 从 room 读取 resolved mode，并写入 AI / tool messages
...
- `aiRoles`：本地全局自定义角色列表
- `roomtalk:ai-settings:${roomId}`：本地按房间保存 `selectedRoleId`、`selectedModel`、`maxContextMessages`
...
| `selectedRole` / custom roles | 本地偏好 | 普通 AI 的个人 prompt 偏好；Coco 不使用普通 role systemPrompt 作为主 system prompt |
...
- `maxContextMessages` 建议写入 Coco turn metadata 或 AI message metadata
- `roleName` 可继续写到 AI message；Coco 场景只作显示，不作为权限或工具语义

---

## Coco Runner 内部架构（调查记录）

Runner 是无状态 Python 进程 `python -m roomtalk_coco_runner`，每次 turn 重新启动。

### 启动流程

1. `CocoSessionService.startTurn()` → `sandboxService.startRunner()` 启动进程
2. 进程环境变量只有模型凭证 + sandbox 配置（不继承 `process.env`）
3. `JsonlCocoRunnerClient` 通过 stdin 写入 JSONL 请求，然后关闭 stdin
...
### 历史消息转换 (`cocoTranscript.ts`)

`buildCocoPriorMessages` 将房间消息转为 Anthropic 对话格式：
