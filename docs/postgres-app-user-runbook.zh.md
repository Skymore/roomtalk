...
本 runbook 用专用 application role 替换过度宽泛的 owner/runtime role，同时保持 startup DDL 兼容。Bundled Compose database 默认用配置的 `POSTGRES_USER` 作为 startup owner；只有在加固当前宿主机或迁移 managed PostgreSQL/RDS 时才有意执行本流程。
...
2. 在不打印 JSON 的前提下，把生产 Keychain 环境中的数据库值切为 application role。
3. 执行 `node scripts/local-production.mjs --profile edge up -d app` reconcile app。
4. 验证 startup schema、room-event listener、loopback/公网 status、新旧 room/message 与受影响 worker。
