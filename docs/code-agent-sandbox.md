...
1. `roomtalk_coco_runner` 是放进 RoomTalk repo，还是放进 Coco repo 作为官方 headless/web adapter？这个问题必须在 Phase 1 前定。
...
6. `acceptEdits` 是否默认允许 Shell，还是需要 RoomTalk 自己做一层 command approval？这个问题必须在真实沙盒前解决。
