...
生产房间 `hXCMDgoefm` 暴露了一个稳定复现的问题：Code Agent 在一次 turn 里交替产生 AI 文本、工具调用、工具结果时，刷新后聊天记录仍然显示为“整段 AI 文本先出现，工具消息后出现”。这说明问题不只是前端实时排序，而是持久化顺序已经错误。
...
| 162 | sandbox_status | `Code Agent runner starting` |
...
RoomTalk 现在的 Code Agent 链路分成四段：

1. 浏览器发起 Code Agent turn。
2. Node `CodeAgentSessionService` 创建 AI placeholder，并按收到的 runner 事件追加消息。
3. Python `roomtalk_code_agent_runner` 调用 Code Agent `Engine.run(...)`。
4. Code Agent engine 内部调用 LLM、执行工具，最后返回 `EngineResult.messages`。
...
sequenceDiagram
    ...
    participant Node as RoomTalk Node
    participant DB as Postgres/Redis Store
    participant Runner as roomtalk_code_agent_runner
    participant Engine as Code Agent engine
    participant Tool as Code Agent Tools

    UI->>Node: ask Code Agent
...
- Code Agent engine 已经能返回完整 `EngineResult.messages`。
- `EngineResult.messages` 里有 assistant `tool_use` block 和 user `tool_result` block。
- RoomTalk runner 可以不改 Code Agent engine，直接扫描最终 messages 生成 RoomTalk 的 `tool_call/tool_result`。
- 但 Code Agent 当时没有完整实时 `tool_result` callback。
...
```mermaid
flowchart TD
    A["Code Agent engine"] -->|"知道 tool_use id/name/input"| B["工具执行"]
    B -->|"知道 output/success/elapsed"| A
    A -->|"应该发 ordered tool events"| C["RoomTalk Runner"]
    C -->|"JSONL text_delta/tool_call/tool_result/final"| D["RoomTalk Node"]
...
| 层 | 应该负责 | 不应该负责 |
|---|---|---|
| Code Agent engine | 按真实执行顺序暴露 text/tool call/tool result 边界 | RoomTalk 的 DB position、Socket 事件 |
| RoomTalk Runner | 把 Code Agent ordered events 转成 JSONL 协议 | 事后猜 AI 文本应该切在哪里 |
| RoomTalk Node | 按 JSONL 到达顺序落库、切 AI segment | 根据完整 AI 文本反推工具发生点 |
| Browser | 展示服务端给出的顺序，实时合并 chunk | 修正已经错误的 DB transcript |

结论：主修点应该是 **Code Agent engine + RoomTalk Runner**。只改前端或只改 Node 都不够。
...
| D. Engine 暴露 ordered tool events，runner 直接转 JSONL | code-agent engine + runner | 是 | 是 | 需要改 Code Agent repo 和重建 artifact | 推荐 |
...
sequenceDiagram
    autonumber
    participant Engine as Code Agent engine
    participant Runner as RoomTalk Runner
    participant Node as RoomTalk Node
...
### 1. Code Agent engine 增加 ordered tool event callback

仓库：`/Users/sky/projects/code-agent-engine`
...
新增一个结构化 callback，建议叫 `on_tool_event`，作为 engine 对外暴露工具边界的唯一正式接口。现有 `on_tool_call` 不建议长期保留；Code Agent CLI 应迁移到 `on_tool_event` 的 `tool_call` 分支。
...
- `on_tool_event` 同时服务 Code Agent CLI 和 RoomTalk runner。
- Code Agent CLI 在收到 `{"type": "tool_call"}` 时执行现在 `_on_tool_call` 的 flush/打印逻辑。
- Code Agent CLI 可以先忽略 `tool_result`，也可以后续用它展示更精确的工具结果状态。
...
Code Agent engine 现在会把连续的只读工具合成一个并发批次。例如模型一次返回了三个工具：
...
### 4. RoomTalk runner 使用 `on_tool_event`

仓库：`/Users/sky/projects/interview-coder/message-system`

目标文件：

- `server/roomtalk_code_agent_runner/roomtalk_code_agent_runner/runner.py`
- `server/roomtalk_code_agent_runner/tests/test_runner.py`
...
### 5. RoomTalk Node 增加回归测试

目标文件：

- `server/src/services/codeAgentSessionService.test.ts`
- 可能涉及 `server/src/services/codeAgentEventMapper.test.ts`
...
### 6. 处理 `Code Agent runner starting` status 的位置问题
...
sandbox_status "Code Agent runner starting" position N+1
```

当 AI placeholder 最后被填上文本，刷新后就会看到 AI 文本在 `Code Agent runner starting` 前面。
...
- `server/src/services/codeAgentEventMapper.ts`
- `server/src/services/codeAgentEventMapper.test.ts`

当前 mapper 已经忽略 `running/ready/complete`，只需要把 `starting` 也加入 ignored。

好处：

- 新 turn 不再出现 `Code Agent runner starting` 插在 AI 和工具之间。
...
3. `new_message` 对 Code Agent tool/AI segment 尽量带 server position。
...
路径：`/Users/sky/projects/code-agent-engine`
...
| 文件 | 改动 |
|---|---|
| `server/roomtalk_code_agent_runner/roomtalk_code_agent_runner/runner.py` | 接 `on_tool_event`，直接发 JSONL tool events，fallback 才 replay |
| `server/roomtalk_code_agent_runner/tests/test_runner.py` | 覆盖 live tool event 顺序、fallback replay、无重复事件 |
| `server/src/services/codeAgentSessionService.test.ts` | 覆盖 text/tool/text 持久化顺序 |
| `server/src/services/codeAgentEventMapper.ts` | 忽略非错误 starting status |
| `server/src/services/codeAgentEventMapper.test.ts` | 更新 status mapper 测试 |
...
### 兼容旧 Code Agent engine

RoomTalk runner 应该用 `inspect.signature(engine.run)` 检测 `on_tool_event`：
...
生产 E2B sandbox 使用 pinned Code Agent artifact。改 Code Agent engine 后，需要更新 artifact：

1. Code Agent repo 提交 engine 改动。
2. RoomTalk 更新 `ops/code-agent-sandbox/artifact.lock.json` 里的 `codeAgentEngine.sourceRef` 和 artifact version。
3. 重新准备 sandbox context：

```bash
node scripts/code-agent/prepare-sandbox-context.mjs \
  --output /tmp/roomtalk-code-agent-sandbox-context
```

4. 重建并发布新的 E2B template。
5. 更新生产 env 的 `CODE_AGENT_E2B_TEMPLATE_ID` / `CODE_AGENT_ARTIFACT_VERSION` / `CODE_AGENT_SOURCE_REF`。

默认从 `artifact.lock.json` 的远端 `sourceRepo` 拉取 pinned commit。只有本地开发验证时才使用 `--engine-repo` 或 `CODE_AGENT_ENGINE_LOCAL_PATH` 覆盖；不要用生产机器上的可变本地源覆盖 pinned artifact。
...
```bash
cd /Users/sky/projects/code-agent-engine
...
pytest server/roomtalk_code_agent_runner
```
...
npm test -- src/services/codeAgentSessionService.test.ts src/services/codeAgentEventMapper.test.ts src/services/jsonlCodeAgentRunner.test.ts
```
...
```bash
cd /Users/sky/projects/interview-coder/message-system/server
RUN_CODE_AGENT_E2B_SMOKE=true npm run smoke:code-agent:e2b
```
...
- `Code Agent runner starting` 不再作为普通聊天消息插入新 turn。

## 验收标准

必须满足：

1. 新 Code Agent turn 的 DB position 是真实 transcript 顺序。
...
- 第二阶段前，少数非 Code Agent optimistic 消息仍按 timestamp fallback。
...
```mermaid
flowchart TD
    A["1. Code Agent engine: on_tool_event"] --> B["2. Code Agent tests: serial/parallel order"]
    B --> C["3. RoomTalk runner: consume on_tool_event"]
...
| 旧 artifact 不兼容 | 生产仍跑旧 Code Agent engine | runner 检测 `on_tool_event`，fallback replay |
...
这不是单纯前端问题，也不是 Node store 随机排序问题。根因是 RoomTalk runner 使用了 Phase 6 MVP 的“文本实时、工具事后 replay”策略，而 Code Agent engine 没有给 RoomTalk 暴露完整 ordered tool events。

正确修复是：

```text
Code Agent engine 暴露 ordered tool_call/tool_result
RoomTalk runner 直接转 JSONL
