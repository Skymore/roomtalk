...
## Current Code Agent Capability Check

Local Code Agent source:
...
- Code Agent does not currently expose a real-time `on_tool_result` callback. Phase 6 MVP will emit text chunks live, then emit tool_call/tool_result events by replaying `EngineResult.messages` after the engine returns. A later Code Agent-side hook can make tool results live without changing the RoomTalk protocol.
...
```mermaid
flowchart LR
  RT["RoomTalk CodeAgentSessionService"] --> SB["Sandbox Service"]
  SB --> RP["Runner Process"]
  RP --> CA["roomtalk_code_agent_runner"]
  CA --> CE["Existing Code Agent engine"]
...
- `CodeAgentSandboxService.startRunner(...)` owns creating and stopping the runner process or remote command session.
- `JsonlCodeAgentRunnerClient` must not spawn a second independent process for the same turn.
...
`FakeCodeAgentRunnerClient` may ignore the context. `JsonlCodeAgentRunnerClient` must require it.
...
MVP implementation can keep fake/local direct-key smoke gated behind explicit env vars, but production `CODE_AGENT_ENABLED=true` with write/Shell capability must require one of:

- RoomTalk model proxy
...
- Add a Python `roomtalk_code_agent_runner` package in the RoomTalk repo.
- Read `CodeAgentRunnerRunRequest` JSONL from stdin.
- Validate schema version and required fields.
- Build Code Agent `AppSettings`, tools, `PermissionChecker`, and `Engine`.
...
- Add a production `JsonlCodeAgentRunnerClient` implementing `CodeAgentRunnerClient`.
- Extend `CodeAgentRunnerClient.run(...)` to receive the already-started `CodeAgentRunnerProcess` and sandbox handle.
- Use the `CodeAgentRunnerProcess` returned by `CodeAgentSandboxService.startRunner(...)`; do not spawn a second command inside the runner client.
- Parse stdout line-by-line with `parseCodeAgentRunnerEventLine`.
- Treat malformed protocol output and process exit as runner errors.
- Keep `FakeCodeAgentRunnerClient` as the default test/E2E path.
...
- Existing CodeAgentSessionService tests still pass.
- Server build passes.
...
- Select fake vs E2B sandbox service from `CODE_AGENT_SANDBOX_PROVIDER`.
- Select fake vs JSONL runner client from `CODE_AGENT_RUNNER_CLIENT`.
...
- `CODE_AGENT_ENABLED=false` blocks backend turns and hides/blocks frontend entry.
...
- Define a reproducible artifact containing:
  - fixed Code Agent source/version
...
- Local smoke can mount `/Users/sky/projects/code-agent-engine` only for development mode.
- Production config uses the pinned artifact.

Implementation note:

- Artifact details live in `docs/code-agent-sandbox-artifact.md`.
- The lock file is `ops/code-agent-sandbox/artifact.lock.json`.
- Python dependencies are pinned and hash-verified in `ops/code-agent-sandbox/requirements.lock`; the Dockerfile loads fixed Code Agent and runner source trees through `PYTHONPATH` to avoid implicit build-time dependency downloads.
...
- RoomTalk now uses the official `e2b` TypeScript SDK for create/connect/list/kill and background command execution. `CODE_AGENT_SANDBOX_PROVIDER=e2b` with `CODE_AGENT_RUNNER_CLIENT=jsonl` requires `E2B_API_KEY` or `E2B_ACCESS_TOKEN` at startup.
...
- Preferred: RoomTalk model proxy.
...
- The finalized model access contract is documented in `docs/code-agent-model-access.md`.
- Proxy mode requires `CODE_AGENT_MODEL_ACCESS_STRATEGY=proxy`, HTTPS `CODE_AGENT_MODEL_PROXY_URL`, and `CODE_AGENT_MODEL_PROXY_TOKEN`.
...
cd client-heroui && npx playwright test --list e2e/code-agent-flows.spec.ts
```

Actual browser E2E and real E2B smoke may require system environment access to local Redis/localhost and external credentials. `npm run smoke:code-agent:e2b` loads `server/.env` when run from `server/` and skips cleanly unless `RUN_CODE_AGENT_E2B_SMOKE=true`, `CODE_AGENT_E2B_TEMPLATE_ID`, E2B credentials, and model access are configured. To force the real sandbox smoke with credentials already stored in `server/.env`, run `RUN_CODE_AGENT_E2B_SMOKE=true npm run smoke:code-agent:e2b`. If the Codex sandbox blocks those, the command, reason, and exact gap must be recorded before moving on.

2026-06-26 merge validation result:

- `cd server && npm run smoke:code-agent:e2b` skipped safely when `RUN_CODE_AGENT_E2B_SMOKE` was not set.
- `cd server && RUN_CODE_AGENT_E2B_SMOKE=true npm run smoke:code-agent:e2b` created a real E2B sandbox, streamed JSONL runner events, completed with a `final` event, and destroyed the sandbox; selected model: `deepseek-v4-pro`.
...
- `CODE_AGENT_ENABLED=false` keeps Code Agent creation hidden in the frontend and rejected by API/socket create paths.
- `CODE_AGENT_ALLOWED_USER_IDS` limits Code Agent creation to explicit client IDs while preserving ordinary chat rooms.
- Feature flag fetch failures fail closed for Code Agent.
- A running Code Agent turn locks the current room input controls until the turn finishes or errors.
