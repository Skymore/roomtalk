...
> 状态：历史设计记录，文中的 scheduler 已被取代。Trigger inventory 与失败
> 分析仍可作为背景阅读，但下文的 suppression window、in-flight 与调度模型
> 不代表当前实现。当前 ownership、epoch/revision、retry 与诊断规则见
> [Room Session Controller 架构](../room-session-controller-design.zh.md)。其中
> Redis presence TTL/heartbeat 仍属于未来可选增强。
