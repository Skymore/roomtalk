...
Object key 按 slug/version/path 分层，使旧版本与新版本不相互覆盖。Finalize 只在所有 object 通过 `HEAD` 校验后更新 manifest/index；房间索引提交失败时恢复上一版 current/version index。旧站点若还没有 version index，会先把当前 manifest 作为第一条历史版本，并在下次发布时自动升级。
