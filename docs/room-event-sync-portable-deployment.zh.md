...
- 收到 `NOTIFY` 后，每个 app 从 PostgreSQL 读取并 hydrate 指定的已提交 sequence；完整通知不超过 `ROOM_EVENT_FAST_PATH_MAX_BYTES`（默认 256 KiB）时，Socket.IO 直接携带 `events`，否则只发 `headSeq`。
...
- page 不连续：整页不应用，重新快照；
- 保留窗口内落后超过 500 events：不再重放最多 100 个默认 page，而是直接 snapshot，再只排空 `snapshotSeq` 之后的 tail；
- IndexedDB v4 保存消息窗口和 `lastAppliedSeq`；
- `beforeMessageId` 只 prepend 旧历史，不移动实时 cursor。

Fast path 只改变延迟，不改变正确性边界。多个 app 实例经 Redis adapter 可能产生重复 payload；客户端忽略已应用 seq，并从 PostgreSQL 补任何 gap。读取/hydration 失败或事件超限时自动回到同一套 durable head-only 路径。
