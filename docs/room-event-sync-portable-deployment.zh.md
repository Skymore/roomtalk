...
`ai_stream_error` 不允许自行创造规范正文，并显式携带 `persisted`。正常错误包含同一条已持久化 safe Message；若终态持久化本身失败，`{ persisted: false }` 会立刻终止浏览器 placeholder 并安排恢复。本地终态 overlay 会保留到 durable final after-image 覆盖它为止。未来 durable reaction mutation 应把 `reactions.upserted` / `reactions.deleted` 加入同一 room sequence；本次切换不凭空实现 reaction 数据模型。
...
一个 per-room 状态机统一拥有 `idle`、`replay`、`replace` 和 `prepend`。Replay/replace recovery 的优先级高于可选分页，因此 prepend 不能让 recovery response 失效。如果删除清空了仍有旧历史的窗口，或者 truncate 后服务端发送 `message_history_invalidated`，控制器会 replace window，而不是根据空 projection 直接设置 `hasMore=false`。

Fast path 只改变延迟，不改变正确性边界。PostgreSQL 把 hint fan-out 到每个 app listener，每个 listener 再用 `io.local` 只通知本机 sockets；Redis adapter 不会放大这条 durable 通知。完整 payload 发出前，实例会用 PostgreSQL 重新鉴权本机 room sockets，并移除已撤权成员。客户端仍忽略已应用 seq，并从 PostgreSQL 补任何 gap。Range 读取不完整或超限时自动回到同一套 durable head-only 路径。
...
GitHub CI 会启动 PostgreSQL 17，并为服务端 suite 强制设置 `ROOM_EVENT_TEST_DATABASE_URL`。Trigger、事务、retention clock 与跨房间 message invariant 测试会真实执行；缺少数据库时不能再静默 skip 后显示全绿。
