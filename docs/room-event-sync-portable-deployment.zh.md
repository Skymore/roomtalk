...
`ai_stream_error` 不允许自行创造规范正文，并显式携带 `persisted`。正常错误包含同一条已持久化 safe Message；若即时终态写入失败，`{ persisted: false }` 会立刻终止浏览器 placeholder，同时进程内 reconciler 用指数退避持续重试同一条 terminal after-image。PostgreSQL owner lease 保护仍在运行的 stream，只有原 owner 过期后其他实例才能接管 orphan。本地终态 overlay 会保留到 durable final after-image 覆盖它为止。未来 durable reaction mutation 应把 `reactions.upserted` / `reactions.deleted` 加入同一 room sequence；本次切换不凭空实现 reaction 数据模型。
...
健康状态不是一个乐观的 boolean。`/api/health/live` 是不依赖下游的进程探针；`/api/status` 与 `/api/health/ready` 会执行真实 RoomTalk 表查询、Redis `PING`、S3-compatible bucket 探测和 Socket adapter 检查。依赖失败时返回 HTTP 503 与 `rooms: null`，而不是错误地报告空数据库。Compose 用 readiness 判断 App container；AWS 应分别配置 liveness/readiness。除此以外还必须监控宿主文件系统和 Docker disk image 容量，因为所有 container 都可能仍在 running，但 Mac 已经没有空间提交 durable write。

GitHub CI 使用 Node 24.18，启动 PostgreSQL 17，并为服务端 suite 强制设置 `ROOM_EVENT_TEST_DATABASE_URL`。Trigger、事务、lease、advisory lock、tombstone、byte boundary、retention clock 与跨房间 message invariant 测试会真实执行；缺少数据库时不能再静默 skip 后显示全绿。
