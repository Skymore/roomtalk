...
- Coco and Codex remain agent backends. RoomTalk does not reimplement their reasoning loops.
...
The default E2B policy pauses an idle sandbox after five minutes, keeps memory, and enables provider auto-resume. Active turns receive a longer TTL. RoomTalk reconnects paused sandboxes instead of treating the timeout as data loss.

### Reusable JSONL daemon

RoomTalk starts one `roomtalk_code_agent_runner.daemon` per sandbox and reuses it for sequential turns. It supports:
