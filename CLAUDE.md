...
- Completed Codex app-server turns are edges in `code_agent_workspace_revisions`. Both the before-turn parent and after-turn revision are valid targets so abandoned branch tips remain reachable. Restore must compute the current/target LCA, undo and redo whole turn edges under a renewed room lease, and move files, the selected Codex boundary, restore audit, and the room revision head as one all-or-nothing operation. A hash conflict aborts the path and rolls already-applied edges back; never fork context or advance the head after a partial file restore.
