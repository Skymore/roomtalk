...
describe('summarizeCodeAgentMessages', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  ...
  it('returns an empty summary when there is no tool activity', () => {
    expect(summarizeCodeAgentMessages([])).toEqual({
      toolCalls: 0,
      toolResults: 0,
      toolErrors: 0,
  ...
  it('counts tool calls, results, errors, and latest tool', () => {
    const summary = summarizeCodeAgentMessages([
      message({
        id: 'call-1',
  ...
  it('loads and validates a RoomTalk-mediated workspace snapshot over socket', async () => {
    const snapshot = {
      roomId: 'room-1',
      backend: 'code-agent',
      source: 'sandbox',
  ...
  it('rejects invalid workspace snapshot responses', async () => {
    requestCodeAgentWorkspaceSnapshotMock.mockResolvedValue({ backend: 'code-agent', source: 'messages', summary: {} });
