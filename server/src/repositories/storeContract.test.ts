...
class MemoryRedis {
  ...
  private rebuildClientMessageIds(messageKey: string, clientMessageIdsKey?: string) {
    ...
    index.set('__roomtalk_index_ready__', '1');
  ...
  async eval(script: string, options: { keys: string[]; arguments: string[] }) {
    ...
    if (script.includes('local dedupeField = ARGV[4]')) {
      ...
      if (dedupeField) {
        ...
        if (!existingMessageId && !index.has('__roomtalk_index_ready__')) {
      ...
      if (dedupeField) {
        ...
        this.hash(clientMessageIdsKey).set('__roomtalk_index_ready__', '1');
      }
...
class StatefulPostgresPool implements PostgresPool, PostgresClient {
  ...
  async query<T = any>(sql: string, params: unknown[] = []): Promise<PostgresQueryResult<T>> {
    ...
    if (/TRUNCATE room_ai_cost_totals, pending_media_uploads, media_assets, room_messages, room_saves, room_members, rooms/.test(compactSql)) {
      this.rooms.clear();
      this.messages.clear();
